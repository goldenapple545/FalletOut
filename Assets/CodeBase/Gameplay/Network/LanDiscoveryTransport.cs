...
namespace CodeBase.Gameplay.Network
{
	...
	/// </summary>
	public class LanDiscoveryTransport : MonoBehaviour
	{
		/// <summary>
		...
		/// <summary>
		/// Данные, которые сервер отправляет в ответ на запрос клиента.
		/// Переопредели, чтобы добавить имя сервера и кол-во игроков.
		/// </summary>
		protected virtual byte[] GetAdvertisementData() => OkBytes;

		/// <summary>
		/// Вызывается на клиенте при получении ответа от сервера.
		/// По умолчанию стреляет ServerFoundCallback.
		/// Переопредели, чтобы разобрать расширенные данные.
		/// </summary>
		protected virtual void OnServerResponseReceived(IPEndPoint endPoint, byte[] data)
		{
			ServerFoundCallback?.Invoke(endPoint);
		}
				...
				while (!cancellationToken.IsCancellationRequested)
				{
					...
					if (completedTask == receiveTask)
					{
						...
						if (receivedSecret == secret)
						{
							...
							byte[] response = GetAdvertisementData();
		...
		private async Task SearchForServersAsync(CancellationToken cancellationToken)
		{
				...
				while (!cancellationToken.IsCancellationRequested)
				{
					...
					if (completedTask == receiveTask)
					{
						...
						if (result.Buffer.Length >= 1 && result.Buffer[0] == 1)
						{
							...
							_mainThreadSynchronizationContext.Post(_ => OnServerResponseReceived(result.RemoteEndPoint, data), null);
						}
						else
						{
			...
			catch (SocketException socketException)
			{
				...
				{
					UnityEngine.Debug.LogException(socketException, this);
				}
			}
			catch (Exception exception)
			{
		...
		{
			if (needLog && NetworkManagerExtensions.CanLog(LoggingType.Common)) 
				UnityEngine.Debug.Log($"[{nameof(LanDiscoveryTransport)}] {message}", this);
		}
		...
		{
			if (needLog && NetworkManagerExtensions.CanLog(LoggingType.Warning))
				UnityEngine.Debug.LogWarning($"[{nameof(LanDiscoveryTransport)}] {message}", this);
		}
		...
		{
			if (needLog && NetworkManagerExtensions.CanLog(LoggingType.Error)) 
				UnityEngine.Debug.LogError($"[{nameof(LanDiscoveryTransport)}] {message}", this);
		}
	}
}
